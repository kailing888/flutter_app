...
class TextureState extends State<TexturePage> {
  @override
  Widget build(BuildContext context) {
    return BasePage("Texture", CameraExampleHome());
  }
}

///Flutter提供了一种基于Texture的图片数据共享机制。
//Texture 组件正是通过textureId与Texture数据关联起来；在Texture组件绘制时，Flutter会自动从内存中找到相应id的Texture数据，然后进行绘制。
//图像数据先在原生部分缓存，然后在Flutter部分再通过textureId和缓存关联起来，最后绘制由Flutter完成
...
class _CameraExampleHomeState extends State<CameraExampleHome>
  ...
  Widget _cameraTogglesRowWidget() {
    ...
    if (MyApp.cameras == null || MyApp.cameras.isEmpty) {
